...
import okhttp3.internal.p013io.FileSystem;
...
public final class Cache implements Closeable, Flushable {
    ...
    Cache(File file, long j, FileSystem fileSystem) {
        this.internalCache = new InternalCache() {
            public C1769Response get(Request request) throws IOException {
                return Cache.this.get(request);
            }

            public CacheRequest put(C1769Response response) throws IOException {
            ...
            public void update(C1769Response response, C1769Response response2) {
    ...
    @Nullable
    public C1769Response get(Request request) {
        try {
            ...
            try {
                ...
                C1769Response response = entry.response(snapshot);
    ...
    /* access modifiers changed from: package-private */
    @Nullable
    public CacheRequest put(C1769Response response) {
        ...
        if (HttpMethod.invalidatesCache(response.request().method())) {
            ...
            return null;
        } else if (!method.equals(WLResourceRequest.GET) || HttpHeaders.hasVaryAll(response)) {
            return null;
        } else {
    ...
    private static final class Entry {
        ...
        private void writeCertList(BufferedSink bufferedSink, List<Certificate> list) throws IOException {
            try {
                ...
                int size = list.size();
                for (int i = 0; i < size; i++) {
                    bufferedSink.writeUtf8(ByteString.m209of(list.get(i).getEncoded()).base64()).writeByte(10);
                }
        ...
        public boolean matches(Request request, C1769Response response) {
            return this.url.equals(request.url().toString()) && this.requestMethod.equals(request.method()) && HttpHeaders.varyMatches(response, this.varyHeaders, request);
        }

        public C1769Response response(DiskLruCache.Snapshot snapshot) {
            String str = this.responseHeaders.get("Content-Type");
            String str2 = this.responseHeaders.get("Content-Length");
            return new C1769Response.Builder().request(new Request.Builder().url(this.url).method(this.requestMethod, (RequestBody) null).headers(this.varyHeaders).build()).protocol(this.protocol).code(this.code).message(this.message).headers(this.responseHeaders).body(new CacheResponseBody(snapshot, str, str2)).handshake(this.handshake).sentRequestAtMillis(this.sentRequestMillis).receivedResponseAtMillis(this.receivedResponseMillis).build();
        }
    }
